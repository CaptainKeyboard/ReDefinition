...
try {
    ...
    function Read-Profile($text) {
        $script:callArgs[0] = $parse.Invoke($null, [object[]]@($text)).GetNode('REDEFINITION_PROFILE')
    ...
    # Mistakes in modules.
    $text = "REDEFINITION_PROFILE`n{`n name = balanced`n title = Balanced`n MODULE`n {`n  name = upscaler`n  enabled = true`n  quality = NativeAA`n }`n MODULE`n {`n  enabled = false`n }`n}"
    $p = Read-Profile $text
    $problems = $callArgs[1]
    $source = $callArgs[0]
    ...
    $text = "REDEFINITION_PROFILE`n{`n name = odd`n title =`n colour = blue`n inherit = defaults`n order = soon`n MODUL`n {`n  name = upscaler`n }`n MOD`n {`n  name = parallax`n  densityMultiplier = 0.5`n }`n}"
    $p = Read-Profile $text
    $problems = $callArgs[1]
    ...
    function Read-Mod($text) {
        $script:callArgs[0] = $parse.Invoke($null, [object[]]@($text)).GetNode('REDEFINITION_MOD')
        $script:callArgs[1] = (New-Object 'System.Collections.Generic.List[string]').PSObject.BaseObject
        return $fromMod.Invoke($null, $script:callArgs)
    }

    $m = Read-Mod "REDEFINITION_MOD`n{`n name = mymod`n detect = MyMod.Settings`n SETTING`n {`n  name = fancyEffects`n  member = MyMod.Settings.fancyEffects`n  default = True`n }`n}"
    $problems = $callArgs[1]
    $s = $m.Settings[0]
    ...
    $m = Read-Mod "REDEFINITION_MOD`n{`n name = profiled`n detect = Profiled.Settings`n SETTING`n {`n  name = detail`n  member = Profiled.Settings.detail`n  kind = Quality`n  default = 3`n }`n SETTING`n {`n  name = glow`n  member = Profiled.Settings.glow`n  kind = Taste`n  default = True`n }`n PROFILE`n {`n  name = low`n  detail = 1`n  detail = 2`n  glow = False`n  ghost = 1`n  INNER`n  {`n  }`n }`n PROFILE`n {`n  name = low`n  build = large`n  detail = 0`n }`n PROFILE`n {`n  detail = 4`n }`n}"
    $problems = $callArgs[1]
    ...
    $m = Read-Mod "REDEFINITION_MOD`n{`n name = every`n detect = Every.Settings`n SETTING`n {`n  name = glow`n  member = Every.Settings.glow`n  kind = Taste`n }`n ALL_PROFILES`n {`n  glow = False`n  ghost = 1`n }`n}"
    $problems = $callArgs[1]
    ...
    # No name at all.
    $none = Read-Profile "REDEFINITION_PROFILE`n{`n title = x`n}"
    $problems = $callArgs[1]
    ...
    foreach ($file in $shipped) {
        ...
        foreach ($node in $root.GetNodes('REDEFINITION_PROFILE')) {
            $shippedProfiles++
            $callArgs[0] = $node
