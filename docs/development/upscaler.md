...
Temporal upscaling in KSP -- FSR 3 in Unity, DLSS and AMD's upscaler DLL in the proxy --
driven by the engine's own motion vectors. Marks: **[src]** read from source, **[doc]** a
vendor's or project's own statement, **[meas]** measured on the development machine,
**[open]** not verified.

## The measurement it rests on: KSP writes object motion vectors

Every temporal upscaler needs colour, depth and motion vectors. Without per-object
motion vectors, rotating parts and propellers would smear.

**[meas]** They are there. With the camera still, any motion in the buffer is object
motion; what tells object motion from camera motion is the shape of the distribution,
not the share of pixels:
...
A flight is no test for this: KSP's floating origin keeps the vessel still relative to
the camera and moves the world, which looks like camera motion. The VAB with the rotation
tool is.

**[meas]** No visible shader on the development install has a `MotionVectors` pass (0 of
64, over 3419 renderers), and the vectors work anyway: Unity's built-in
`Hidden/Internal-MotionVectors` covers motion by transform, and skinned meshes where
`SkinnedMeshRenderer.skinnedMotionVectors` is on **[src]**. `SkinnedMotionVectors`
switches that on for kerbals and flags while the rig runs. Geometry a material's vertex
function moves gets no motion vectors.

## FSR3Unity on Unity 2019.4

`src/Fsr3` and the FSR shaders in `unity/Assets/ReDefinition/Shaders` are
[FSR3Unity](https://github.com/ndepoel/FSR3Unity) (MIT, Nico de Poel) at FSR 3.1.3, a C#
reimplementation of AMD's FidelityFX backend with AMD's FSR 3 compute shaders, adapted
for Unity 2019.4 and KSP.

FSR3Unity needs Unity 2020.1 for `multi_compile` in compute shaders; KSP is 2019.4.18f1,
and shader bundles must come from exactly that version. Of the seven keywords, six are
set once at initialisation and one (`APPLY_SHARPENING`) decides between two accumulate
passes, so the combination is known and baked in: one variant per shader, a second
accumulate pass with sharpening, and each set once with `HDR_COLOR_INPUT` and once
without (`_ldr`). The rig picks the set by the camera: KSP renders without HDR in the
editors and with it in flight.

**The shaders** are made from FSR3Unity's by `tools/port_fsr3_shaders.py`. It bakes the
keywords in, turns the `cbuffer` blocks into `StructuredBuffer`s --
`SetComputeConstantBufferParam` also arrived only in 2020.1 -- and adds FSR 3.1.4's four
tuning constants, AMD's `FFX_API_CONFIGURE_UPSCALE_KEY_*`, at AMD's defaults. The getter
functions and passes stay as they are. A structured buffer rather than global uniforms:
...
**The C# runtime** is adapted in the source; the larger changes are marked
`ReDefinition:` there.
...
Both run in the `dxgi.dll` proxy, on the same inputs as FSR 3: the rig writes a packet
per frame into its dispatch buffer, executed from `OnRenderImage`, and a render event
hands it to the proxy on Unity's render thread (`NativeUpscalerLink`, `PacketRing`).
Jitter and motion vector scale are FSR's, in pixels at render size (NVIDIA's DLSS
Programming Guide 3.6.1, 3.7.3; AMD's `ffx_upscale.h`) **[doc]**.

* **DLSS** (`Dlss.h`, `DlssBridge`) runs through NGX on Unity's Direct3D 11 device, with
  auto exposure, and the render preset the player chose (Default lets the DLSS library
  choose). DLSS sharpens nothing itself, so with sharpness above 0 it writes into the
  input of FSR 3's RCAS pass (`RcasSharpener`), which writes the output. DLSS renders at
  the size it asks for; another size makes a new rig.
* **AMD's DLL** (`AmdUpscaler.h`, `AmdUpscalerBridge`) runs on the proxy's Direct3D 12
  device: colour, depth and motion vectors are copied into textures both devices share,
  AMD's upscaler runs on the Direct3D 12 queue, and the result is copied back into
  Unity's output, with a shared fence between the two. The DLL decides which upscaler
  runs: FSR 4 where the DLL and the GPU have it, otherwise FSR 3.1.
* **The proxy's state**: 0 before the first frame, 1 while it upscales, -1 once
  something stopped it, -2 when that stands until the GPU, driver or DLL changes. The
  proxy's output is shown only once it has reported an upscaled frame; before that the
  image without upscaling. A technique that stays stopped gives way to FSR 3: the
  add-on builds a new rig with FSR 3 in its place, and the diagnostics window offers to
...
* **The camera stack.** `GalaxyCamera` paints the stars, `Camera ScaledSpace` the distant
  planets, `Camera 01` and `Camera 00` the near scene -- all into one frame buffer, with
  `InternalCamera`, `Main Camera` and others in other scenes (the list follows KerbalVR,
  which redirects the same stack). An image effect on one camera would make Unity render
  it into its own texture and lose what the cameras below drew. So the whole 3D stack
  renders into one shared render texture (`CameraRedirect`), and a presenter camera
  behind it scales the result into the frame buffer before the UI cameras
  (`UpscalerPresenter`). `FXCamera` and `FXDepthCamera` are not redirected: they are not
  part of the image.
* **The jitter puts back the projection it found.** Once a script assigns a camera's
  ...
  projection Unity computed is Unity's again, and one a script set -- KSP's
  `CameraOffCenter` in the editors -- is written back, unless that script changed it
  during the render.
...
* **Depth comes from `BuiltinRenderTextureType.ResolvedDepth`.** `.Depth` delivers
  nothing in the deferred path; the depth sub-element of the camera target does not exist
  in 2019.4.
...
* **Game-wide quality settings** (`QualityOverrides`). While the upscaler runs, the LOD
  bias is compensated by the same factor as the mipmap bias (Unity picks LODs by covered
  pixels), MSAA is off, and anisotropic filtering is forced on so the negative mipmap bias
  does not shimmer. Shadow distance and cascades are left alone. KSP's `SetQualityLevel`
  resets these; `OnGameSettingsApplied` puts them back. A value KSP writes there other
  than ReDefinition's -- MSAA from KSP's settings screen, say -- becomes the one restored
  when the upscaler stops.
* **Mipmap bias** (`KspMipmapBias`) touches only textures on renderers a redirected camera
  sees.

## Other mods in the image

### Temporal and spatial filters -- `HostStack`

An upscaler is a temporal filter; a second one in front of it averages the image twice,
and both jitter the same projection. `HostStack` reads the other mods' state by
reflection -- no mod is a reference -- and, while a graphics profile is chosen, puts it
into what the upscaler needs:
...
* Scatterer's `TemporalAntiAliasing` and SMAA off, and Deferred's SMAA in the editors.
  ...
  `BeforeImageEffects`, so the upscaler would receive smoothed edges -- what it
  reconstructs from. The buffer outlives the component and is taken off too; Deferred
  adds a fresh copy on every editor scene load, so the editors are checked once a second.
...
shift the whole image **[meas]**: the component is switched off.
...
All three are cleaned up. What was found is remembered, reasserted after scene changes
(Scatterer rebuilds its components), and restored only where the value is still the one
written -- a newer choice by the player or another mod stands.
...
* **Motion vectors** (`CloudMotionVectors`): EVE's own
  ...
  Unity's at the capture the way Scatterer's TAA blends them: the clouds' where they
  cover the pixel, over the sky by their transmittance, over geometry once less than a
  tenth of it is left.
...
and effects other mods inject before the built-in stack on the redirected layer, and a
layer of ReDefinition's, on a camera that never renders, draws the rest over the
upscaler's output from the same volumes. Which layer draws what is decided right after
PostProcessing blends a layer's settings (a Harmony postfix on `UpdateSettings`). Only
while the image the upscaler receives is HDR: in an 8-bit target the highlights bloom and
tonemapping need are clamped first. Depth of field and motion blur after the upscaler get
the rig's copies of depth and motion vectors. Dithering happens once, on the final image.
Debug switch *TUFX after upscaling*, on by default.

### Masks (FSR 3)

`UpscalerMasks`, both off by default:

* **Transparency and composition** -- what motion vectors do not follow though nothing is
  blended: EVE's clouds by their transmittance and fade, Scatterer's ocean by its ocean
  G-buffer depth.
* **Reactive** -- what is blended over the scene: particles and their trails, engine
  effects, re-entry. Either drawn from the transparent renderers in view with the mask
  shader, occluded by a copy of the scene's depth, or FSR's own generator from an
  opaque-only copy and the image after the transparent queue.

Recorded at the scene camera's `AfterForwardAlpha`. AMD's guidance for both:
`super-resolution-upscaler.md` of the FidelityFX SDK, "Reactive mask" and "Transparency
and composition mask".

## What it buys, on the development machine
...
AMD quotes. In flight the GPU sits at 90-97 % and the CPU at 30-40 %: saturated by work
that does not scale with output resolution -- Scatterer's shadow cascades at 8192, EVE's
...
hardware limited by pixel fill rate the smaller modes would pay; not measured.

## Camera cuts

The history is reset whenever the flight camera's target, its parent or the kerbal of the
IVA view differs from the last look -- taken before the frame's first scene camera culls
and again when the rig dispatches -- and whenever a mod reports a cut
(`Frame.RequestHistoryReset`, [shared-foundation.md](shared-foundation.md)); a change of
camera mode rebuilds the rig, since entering IVA enables `InternalCamera`. KSP's
camera events are not used: they arrive after the rig's update, twice, when nothing cut,
or not at all. Not on a floating origin shift: KSP shifts every frame above a speed
...
In the diagnostics window's *Debug* tab ([player/settings-window.md](../player/settings-window.md)):
