...
extern "C"
{
    ...
    {
        ksp::FrameGeneration::Get().RegisterInputs(
    ...
    {
        ksp::FrameGeneration::Get().SetEnabled(enabled != 0);
    }
    ...
    {
        return ksp::Config().frameGeneration ? 1 : 0;
    }
    ...
    {
        return ksp::FrameGenerationSwapChainPresent() ? 1 : 0;
    }
    ...
    __declspec(dllexport) int KspFgTechnique(int* multiplier)
    {
        if (multiplier != nullptr)
            *multiplier = ksp::FrameGeneration::Get().StreamlineMultiplier();
        return ksp::FrameGenerationTechnique();
    }
    ...
    {
        return ksp::FrameGeneration::Get().StreamlineVsyncSupported() ? 1 : 0;
    }
    ...
    {
        ksp::NvidiaGpuInfo info;
        const bool known = ksp::KnownNvidiaGpu(info);
    ...
    __declspec(dllexport) void KspNvidiaDirectories(wchar_t* dlss, wchar_t* streamline, int size)
    {
        if (size <= 0)
            return;
        if (dlss != nullptr)
            wcsncpy_s(dlss, static_cast<size_t>(size), ksp::DlssDirectory().c_str(), _TRUNCATE);
        if (streamline != nullptr)
            wcsncpy_s(streamline, static_cast<size_t>(size), ksp::StreamlineDirectory().c_str(), _TRUNCATE);
    }
    ...
    {
        return ksp::FrameGeneration::Get().ContextFailing();
    }
    ...
    {
        ksp::FrameGeneration::Get().RetryContext();
    }
    ...
    {
        ksp::FrameGeneration::Get().RequestCheck();
    }
    ...
    __declspec(dllexport) int KspFgCounters(unsigned int* rendered, unsigned int* presented)
    {
        uint32_t r = 0;
        uint32_t p = 0;
        const bool valid = ksp::FrameCounters(r, p);
    ...
    {
        ksp::LoadMonitor::Get().RegisterMainThread();
    }
    ...
    {
        if (!ksp::LoadMonitor::Get().Allowed())
            return -1;

        ksp::LoadSample sample;
        const bool valid = ksp::LoadMonitor::Get().Latest(sample);
    ...
    {
        return static_cast<unsigned int>(sizeof(ksp::FramePacket));
    }
    ...
    {
        if (eventId == 1)
            ksp::FrameGeneration::Get().OnFramePacket(data);
        else if (eventId == 2)
            ksp::Dlss::Get().OnPacket(data);
        else if (eventId == 3)
            ksp::Dlss::Get().OnRelease();
        else if (eventId == 4)
            ksp::AmdUpscaler::Get().OnPacket(data);
        else if (eventId == 5)
            ksp::AmdUpscaler::Get().OnRelease();
        else if (eventId == 6)
            ksp::Dlss::Get().OnSizeQuery(data);
        else if (eventId == 20)
            ksp::D3d12Compute::Get().OnCreatePass(data);
        else if (eventId == 21)
            ksp::D3d12Compute::Get().OnDestroyPass(data);
        else if (eventId == 22)
            ksp::D3d12Compute::Get().OnDispatch(data);
        else if (eventId == 23)
            ksp::D3d12Compute::Get().OnReleaseTexture(data);
    }
    ...
    {
        return static_cast<unsigned int>(sizeof(ksp::DlssPacket));
    }
    ...
    {
        return static_cast<unsigned int>(sizeof(ksp::AmdUpscalerPacket));
    }

    __declspec(dllexport) int KspAmdUpscalerStatus(char* buffer, int size)
    {
        return ksp::AmdUpscaler::Get().Status(buffer, size);
    }
    ...
    {
        return ksp::AmdUpscaler::Get().Attached() ? 1 : 0;
    }

    __declspec(dllexport) int KspDlssStatus(char* buffer, int size)
    {
        return ksp::Dlss::Get().Status(buffer, size);
    }
                                                  ...
                                                  unsigned int* optimal, unsigned int* minimum, unsigned int* maximum)
    {
        ...
        uint32_t lo[2] = {};
        uint32_t hi[2] = {};
        const bool valid = ksp::Dlss::Get().RenderSizes(outputWidth, outputHeight, quality, o, lo, hi);
        for (int i = 0; i < 2; ++i)
        {
    ...
    {
        ksp::D3d12Compute::Capabilities found;
        const bool available = ksp::D3d12Compute::Get().Available(found);
    ...
    {
        return static_cast<unsigned int>(sizeof(ksp::D3d12CreatePacket));
    }

    __declspec(dllexport) unsigned int KspD3d12DispatchPacketSize()
    {
        return static_cast<unsigned int>(sizeof(ksp::D3d12DispatchPacket));
    }
    ...
    {
        int size = 0;
        const int result = ksp::CompileComputeShader(source, sourceLength, path, entry, bytecode, capacity, size,
    ...
    {
        return ksp::D3d12Compute::Get().PassStatus(pass, buffer, size);
    }

    __declspec(dllexport) int KspD3d12Status(char* buffer, int size)
    {
        return ksp::D3d12Compute::Get().Status(buffer, size);
    }
    ...
    __declspec(dllexport) int KspFgStatus(char* buffer, int size)
    {
        ...
        ksp::FrameGeneration& fg = ksp::FrameGeneration::Get();
    ...
    __declspec(dllexport) int KspFgLastHudLessCheck(float* direct, float* mirrored)
    {
        float d = -1.0f;
        float m = -1.0f;
        ksp::FrameGeneration::Get().LastHudLessCheck(d, m);
