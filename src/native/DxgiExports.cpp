...
namespace
{
    ...
    void LoadRealDxgi()
    {
        std::call_once(g_loadOnce, [] {
            ...
            ksp::LogLine(g_realDxgi != nullptr ? "System dxgi.dll loaded"
        ...
        IDXGIOutput* restrictToOutput, IDXGISwapChain1** swapChain)
    {
        ...
        if (device != nullptr && SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&dxgiDevice)))
            ...
            ksp::NoteSwapChainAdapter(adapterDesc.AdapterLuid);

        if (ksp::Config().enabled && desc != nullptr && swapChain != nullptr && hwnd != nullptr)
        {
            ...
            if (ksp::Config().measureOnly)
            {
                ...
                if (SUCCEEDED(hr) && *swapChain != nullptr)
                {
                    ...
                    if (SUCCEEDED(ksp::SwapChainProxy::CreateMeasuring(device, real, *desc, &wrapper)))
                    {
            ...
            const HRESULT hr = ksp::SwapChainProxy::Create(
    ...
    void HookFactory(IDXGIFactory2* factory)
    {
        ...
        std::call_once(g_hookOnce, [factory] {
            ...
            {
                ksp::LogLine("VirtualProtect on the factory vtable failed");
                return;
            }
            ...
            ksp::LogLine("CreateSwapChainForHwnd hooked");
        });
    }
...
BOOL APIENTRY DllMain(HMODULE module, DWORD reason, LPVOID reserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        ksp::LogOpen();
        ksp::LogLine("ReDefinition dxgi proxy attached");
        ksp::LoadConfig();
        ksp::LoadMonitor::Get().SetAllowed(ksp::Config().enabled);
        break;
